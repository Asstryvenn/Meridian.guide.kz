...
export function ActivityRecommender() {
  ...
  const [selectedField, setSelectedField] = useState<FieldOfStudy>(
    profile.fields[0] || tx("Computer Science")
  );
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-6 rounded-3xl bg-panel/85 border border-[#589C80]/30 shadow-2xl backdrop-blur-xl">
        <div className="space-y-1">
          <div className="flex items-center gap-2.5">
            <Sparkles size={22} className="text-amber-ink" />
        ...
        <div className="flex items-center gap-2">
          <label className="text-xs font-mono font-semibold text-amber-ink uppercase">
      ...
      {loading ? (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          {[1, 2, 3].map((i) => (
            <div
              key={i}
              className="h-64 rounded-3xl bg-panel/50 border border-[#589C80]/20 animate-pulse p-6"
            />
          ))}
        </div>
      ) : (
            ...
            {recommendations.map((rec) => {
              ...
              return (
                <motion.div
                  ...
                  className="rounded-3xl bg-panel/90 border border-[#589C80]/30 hover:border-[#EBAE29] transition-all duration-300 p-6 flex flex-col justify-between space-y-4 shadow-xl backdrop-blur-xl"
                >
                  <div className="space-y-3">
                    <div className="flex items-center justify-between gap-2">
                      <span className="px-2.5 py-1 rounded-full text-[10px] font-mono font-bold uppercase tracking-wider bg-[#589C80]/20 text-green-ink border border-[#589C80]/40">
                        {rec.category}
                      </span>
                      <span className="px-2 py-0.5 rounded-md text-[10px] font-mono font-extrabold text-amber-ink bg-[#EBAE29]/15 border border-[#EBAE29]/30">
                    ...
                    <div>
                      ...
                      <p className="text-xs font-mono text-green-ink mt-0.5">
                        {rec.role}
                      </p>
                    </div>
