...
export function applyImport(profile: StudentProfile, result: ImportResult): { profile: StudentProfile; changes: ImportChange[] } {
  ...
  set("fullName", found.fullName, "Name");
  set("country", found.country, "Country");
  set("school", found.school, "School");
  set("graduationYear", found.graduationYear, "Graduation year");
  set("gradingSystem", found.gradingSystem, "Grading system");
  set("gpa", found.gpa, "Average grade");
  set("sat", found.sat, "SAT");
  set("act", found.act, "ACT");
  set("ib", found.ib, "IB");
  set("aLevels", found.aLevels, "A-Levels");
  set("apCourses", found.apCourses, "AP courses");
  set("ielts", found.ielts, "IELTS");
  set("toefl", found.toefl, "TOEFL");
  set("duolingo", found.duolingo, "Duolingo");
  if (found.fields.length) set("fields", Array.from(new Set([...profile.fields, ...found.fields])).slice(0, 3), "Fields", found.fields.join(", "));
  if (found.interestsNote && !profile.interestsNote) set("interestsNote", found.interestsNote, "Interests");
  ...
  if (added.length) {
    ...
    changes.push({ label: "Activities", value: `${added.length} added and sorted by level` });
  }
...
export function extractFromText(text: string): ImportResult {
  ...
  return {
    documentType: "other",
    summary: "Scores detected in plain text without AI.",
    profile: {
      fullName: null,
      country: null,
      school: null,
    ...
    activities: [],
    warnings: ["Only test scores were detected. Add an OpenAI key to read full documents and activities."],
  };
}
